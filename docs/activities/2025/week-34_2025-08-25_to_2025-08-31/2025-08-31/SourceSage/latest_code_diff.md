...
-
-# aira --mode sourcesage commit --ss-model-name="gemini/gemini-1.5-flash-latest" 
...
+SourceSageは、リポジトリの構造と内容を包括的に分析し、AIフレンドリーなドキュメントを生成するツールです。主にRepository_summary.mdの生成とRELEASE_REPORTの作成機能を提供します。
 ...
 >[!IMPORTANT]
@@ -48,25 +48,13 @@ SourceSageは、AIを活用してソフトウェア開発を支援するツー
...
@@ -75,73 +63,193 @@ SourceSageは、AIを活用してソフトウェア開発を支援するツー
 - 【2024/03/29】 初期リリース
...
+name = "sourcesage"
+version = "6.2.0"
+description = "Repository analysis tool that generates AI-friendly documentation"
+readme = "README.md"
+requires-python = ">=3.8"
+license = {text = "MIT"}
+authors = [
+    {name = "Sunwood AI Labs", email = "sunwood.ai.labs@gmail.com"},
+]
+keywords = ["documentation", "repository", "analysis", "markdown", "ai"]
+classifiers = [
...
-    ],
-)
diff --git a/sourcesage/__init__.py b/sourcesage/__init__.py
index e69de29..c7fad4c 100644
--- a/sourcesage/__init__.py
+++ b/sourcesage/__init__.py
@@ -0,0 +1,17 @@
+try:
...
diff --git a/sourcesage/cli.py b/sourcesage/cli.py
index 7a8d2fc..6485ab2 100644
--- a/sourcesage/cli.py
+++ b/sourcesage/cli.py
@@ -1,25 +1,29 @@
 # sourcesage\cli.py
...
@@ -27,83 +31,116 @@ def add_arguments(parser):
     # ==============================================
     # 基本設定
     #
...
+    parser.add_argument('--repo-path', type=str, default="", help='gitリポジトリへのパス（省略時はカレントディレクトリ）')
...
+    parser.add_argument('--report-sections', type=str, nargs='+',
...
+    parser.add_argument('-h', '--help', action='store_true', help='このヘルプを表示して終了')
+
+
...
+    # Modes
+    modes = Text()
+    modes.append("--ss-mode all ", style="green"); modes.append("(デフォルト)\n")
+    modes.append("--ss-mode Sage ", style="green"); modes.append("(Repository Summaryのみ)\n")
+    modes.append("--ss-mode GenerateReport ", style="green"); modes.append("(Release Reportのみ)\n")
...
+        return fallback
+
+    core_tbl.add_row("--ss-output", str(_get_default("ss_output")), "生成ファイルの出力ディレクトリ")
+    core_tbl.add_row("--repo", str(_get_default("repo")), "解析対象のリポジトリパス")
+    core_tbl.add_row("--ignore-file", str(_get_default("ignore_file")), "無視パターンファイルのパス")
+    core_tbl.add_row("--language-map", str(_get_default("language_map")), "言語マップJSONのパス")
+    core_tbl.add_row("--ss-mode", "[all|Sage|GenerateReport]", "実行モード")
+    console.print(core_tbl)
+
+    # Release report options
+    rel_tbl = Table(title="Release Report Options", show_header=True, header_style="bold magenta")
+    rel_tbl.add_column("Option")
+    rel_tbl.add_column("Default", style="dim")
+    rel_tbl.add_column("Description")
+    rel_tbl.add_row("--repo-path", str(_get_default("repo_path")), "gitリポジトリのルートパス")
+    rel_tbl.add_row("--git-fetch-tags", str(_get_default("git_fetch_tags")), "gitタグ取得コマンド")
+    rel_tbl.add_row("--git-tag-sort", str(_get_default("git_tag_sort")), "gitタグソートコマンド")
+    rel_tbl.add_row("--git-diff-command", str(_get_default("git_diff_command")), "git diff コマンド")
+    rel_tbl.add_row("--report-title", str(_get_default("report_title")), "レポートタイトル")
+    rel_tbl.add_row("--report-sections", str(_get_default("report_sections")), "含めるセクション")
+    rel_tbl.add_row("--ss-output-path", str(_get_default("ss_output_path")), "レポート出力先フォルダ")
+    rel_tbl.add_row("--report-file-name", str(_get_default("report_file_name")), "レポートファイル名")
...
@@ -119,58 +156,38 @@ def run(args=None):
     # -----------------------------------------------
     # SourceSageの実行
...
+            sourcesage = SourceSage(args.ss_output, args.repo, args.ignore_file, args.language_map)
+            sourcesage.run()
...
+                diff, latest_tag, previous_tag,
+                args.report_title, args.report_sections,
+                output_path, repo_path=repo_root
+            )
...
diff --git a/sourcesage/config/ISSUES_RESOLVE/ISSUES_RESOLVE_TEMPLATE.md b/sourcesage/config/ISSUES_RESOLVE/ISSUES_RESOLVE_TEMPLATE.md
deleted file mode 100644
index 66bb534..0000000
--- a/sourcesage/config/ISSUES_RESOLVE/ISSUES_RESOLVE_TEMPLATE.md
+++ /dev/null
@@ -1,44 +0,0 @@
-
...
-{{sourcesage_md}}
-
diff --git a/sourcesage/config/STAGE_INFO/STAGE_INFO_AND_ISSUES_TEMPLATE.md b/sourcesage/config/STAGE_INFO/STAGE_INFO_AND_ISSUES_TEMPLATE.md
deleted file mode 100644
index a4b6c3f..0000000
--- a/sourcesage/config/STAGE_INFO/STAGE_INFO_AND_ISSUES_TEMPLATE.md
+++ /dev/null
@@ -1,51 +0,0 @@
-下記はissuesの情報です
-
...
-\```
\ No newline at end of file
diff --git a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE.md b/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE.md
deleted file mode 100644
index c8b85d1..0000000
--- a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE.md
+++ /dev/null
@@ -1,45 +0,0 @@
-
-
...
-\```
\ No newline at end of file
diff --git a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_EMOJI.md b/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_EMOJI.md
deleted file mode 100644
index a291808..0000000
--- a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_EMOJI.md
+++ /dev/null
@@ -1,45 +0,0 @@
-
-
...
-\```
\ No newline at end of file
diff --git a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_GAIAH_B.md b/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_GAIAH_B.md
deleted file mode 100644
index 6621cc2..0000000
--- a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_GAIAH_B.md
+++ /dev/null
@@ -1,80 +0,0 @@
-
-下記はgitはStageの情報です
...
-\```
-
diff --git a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_GAIAH_B_EN.md b/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_GAIAH_B_EN.md
deleted file mode 100644
index 22e1bdd..0000000
--- a/sourcesage/config/STAGE_INFO/STAGE_INFO_TEMPLATE_GAIAH_B_EN.md
+++ /dev/null
@@ -1,77 +0,0 @@
-
-下記はgitはStageの情報です
...
-\```
-
diff --git a/sourcesage/config/constants.py b/sourcesage/config/constants.py
index a99cb53..f5b3c69 100644
--- a/sourcesage/config/constants.py
+++ b/sourcesage/config/constants.py
@@ -1,45 +1,21 @@
 import os
...
diff --git a/sourcesage/config/language_map.json b/sourcesage/config/language_map.json
index 464b726..b3df464 100644
--- a/sourcesage/config/language_map.json
+++ b/sourcesage/config/language_map.json
@@ -2,14 +2,20 @@
     ".py": "python",
     ".js": "javascript",
     ".ts": "typescript",
...
\ No newline at end of file
diff --git a/sourcesage/core.py b/sourcesage/core.py
index 513cbd2..bb5338f 100644
--- a/sourcesage/core.py
+++ b/sourcesage/core.py
@@ -1,98 +1,36 @@
 # sourcesage\core.py
 import os
...
diff --git a/sourcesage/logging_utils.py b/sourcesage/logging_utils.py
new file mode 100644
index 0000000..f47c857
--- /dev/null
+++ b/sourcesage/logging_utils.py
@@ -0,0 +1,44 @@
+from loguru import logger
+from rich.console import Console
+from rich.text import Text
...
+    return console
diff --git a/sourcesage/modules/ChangelogGenerator.py b/sourcesage/modules/ChangelogGenerator.py
deleted file mode 100644
index 66d49ac..0000000
...
\ No newline at end of file
diff --git a/sourcesage/modules/ChangelogUtils.py b/sourcesage/modules/ChangelogUtils.py
deleted file mode 100644
index 3df6971..0000000
--- a/sourcesage/modules/ChangelogUtils.py
+++ /dev/null
@@ -1,35 +0,0 @@
...
-        file.write(f"- **メッセージ**: \n\n    {formatted_message}\n\n")
\ No newline at end of file
diff --git a/sourcesage/modules/CommitCraft.py b/sourcesage/modules/CommitCraft.py
deleted file mode 100644
index e471820..0000000
--- a/sourcesage/modules/CommitCraft.py
+++ /dev/null
@@ -1,177 +0,0 @@
-import os
...
diff --git a/sourcesage/modules/DiffChangelogGenerator.py b/sourcesage/modules/DiffChangelogGenerator.py
deleted file mode 100644
index 6cf6529..0000000
--- a/sourcesage/modules/DiffChangelogGenerator.py
+++ /dev/null
@@ -1,123 +0,0 @@
...
\ No newline at end of file
diff --git a/sourcesage/modules/DiffReport/__init__.py b/sourcesage/modules/DiffReport/__init__.py
new file mode 100644
index 0000000..1d5a3c2
--- /dev/null
+++ b/sourcesage/modules/DiffReport/__init__.py
@@ -0,0 +1,8 @@
...
+]
+
diff --git a/sourcesage/modules/DiffReport/git_diff.py b/sourcesage/modules/DiffReport/git_diff.py
new file mode 100644
index 0000000..8e1846d
--- /dev/null
+++ b/sourcesage/modules/DiffReport/git_diff.py
@@ -0,0 +1,34 @@
+from loguru import logger
+from ..GitCommander import run_command
+
...
diff --git a/sourcesage/modules/DiffReport/markdown_report.py b/sourcesage/modules/DiffReport/markdown_report.py
new file mode 100644
index 0000000..ebc7c5e
--- /dev/null
+++ b/sourcesage/modules/DiffReport/markdown_report.py
@@ -0,0 +1,162 @@
+from loguru import logger
+import os
+try:
...
diff --git a/sourcesage/modules/DocuMind.py b/sourcesage/modules/DocuMind.py
deleted file mode 100644
index 37a766b..0000000
--- a/sourcesage/modules/DocuMind.py
+++ /dev/null
@@ -1,173 +0,0 @@
...
diff --git a/sourcesage/modules/DocuSum/.SourceSageignore b/sourcesage/modules/DocuSum/.SourceSageignore
deleted file mode 100644
index a029c83..0000000
--- a/sourcesage/modules/DocuSum/.SourceSageignore
+++ /dev/null
@@ -1,54 +0,0 @@
-# バージョン管理システム関連
-.git/
...
-.venv
-
diff --git a/sourcesage/modules/DocuSum/docusum.py b/sourcesage/modules/DocuSum/docusum.py
index 267e24b..c062f8a 100644
--- a/sourcesage/modules/DocuSum/docusum.py
+++ b/sourcesage/modules/DocuSum/docusum.py
@@ -1,8 +1,7 @@
 import os
...
+            raise
     ...
     def analyze_repository(self):
         """リポジトリの分析結果を生成する"""
diff --git a/sourcesage/modules/DocuSum/stats_collector.py b/sourcesage/modules/DocuSum/stats_collector.py
index 62fe87b..9d66dd7 100644
--- a/sourcesage/modules/DocuSum/stats_collector.py
+++ b/sourcesage/modules/DocuSum/stats_collector.py
...
+        return sorted(file_stats, key=lambda x: x['lines'], reverse=True)
     ...
     def collect_language_stats(self, file_stats):
         """言語別の統計情報を収集する"""
diff --git a/sourcesage/modules/EnvFileHandler.py b/sourcesage/modules/EnvFileHandler.py
deleted file mode 100644
index 2ac9b43..0000000
--- a/sourcesage/modules/EnvFileHandler.py
+++ /dev/null
@@ -1,39 +0,0 @@
...
\ No newline at end of file
diff --git a/sourcesage/modules/GitHubIssueRetrieve.py b/sourcesage/modules/GitHubIssueRetrieve.py
deleted file mode 100644
index c9ce650..0000000
--- a/sourcesage/modules/GitHubIssueRetrieve.py
+++ /dev/null
@@ -1,25 +0,0 @@
...
-    retriever.run()
\ No newline at end of file
diff --git a/sourcesage/modules/GitHubUtils.py b/sourcesage/modules/GitHubUtils.py
deleted file mode 100644
index 63cb116..0000000
--- a/sourcesage/modules/GitHubUtils.py
+++ /dev/null
@@ -1,41 +0,0 @@
...
-        return staged_diff
\ No newline at end of file
diff --git a/sourcesage/modules/IssueWize.py b/sourcesage/modules/IssueWize.py
deleted file mode 100644
index 6e5eb6b..0000000
--- a/sourcesage/modules/IssueWize.py
+++ /dev/null
@@ -1,241 +0,0 @@
...
diff --git a/sourcesage/modules/IssuesToMarkdown.py b/sourcesage/modules/IssuesToMarkdown.py
deleted file mode 100644
index d8852d6..0000000
--- a/sourcesage/modules/IssuesToMarkdown.py
+++ /dev/null
@@ -1,57 +0,0 @@
-import json
-import os
...
\ No newline at end of file
diff --git a/sourcesage/modules/ReleaseDiffReportGenerator.py b/sourcesage/modules/ReleaseDiffReportGenerator.py
deleted file mode 100644
index 4c26ebb..0000000
--- a/sourcesage/modules/ReleaseDiffReportGenerator.py
+++ /dev/null
@@ -1,139 +0,0 @@
...
-    main()
\ No newline at end of file
diff --git a/sourcesage/modules/StageInfoGenerator.py b/sourcesage/modules/StageInfoGenerator.py
deleted file mode 100644
index 0d89b52..0000000
--- a/sourcesage/modules/StageInfoGenerator.py
+++ /dev/null
@@ -1,82 +0,0 @@
...
-    )
-    stage_info_generator.run()
diff --git a/sourcesage/modules/StagedDiffGenerator.py b/sourcesage/modules/StagedDiffGenerator.py
deleted file mode 100644
index 9bb3d85..0000000
--- a/sourcesage/modules/StagedDiffGenerator.py
+++ /dev/null
@@ -1,73 +0,0 @@
...
+]
+dependencies = [
+    { name = "coverage", version = "7.10.5", source = { registry = "https://pypi.org/simple" }, extra = ["toml"], marker = "python_full_version >= '3.9'" },
+    { name = "pluggy", version = "1.6.0", source = { registry = "https://pypi.org/simple" }, marker = "python_full_version >= '3.9'" },
